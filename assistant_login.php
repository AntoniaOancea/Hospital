...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    $mail = $_POST['mail'];
    $pass_query = mysqli_query($conn, "SELECT password, id_assistant FROM assistants WHERE mail ='" . $mail . "'");
    if (!$pass_query) {
        ...
        if (mysqli_num_rows($pass_query) > 0) {
            ...
            if ($_POST['password'] == $pass) {
                ...
                header("Location:assistant.php?edit=".$id);
                die;
            } else {
                echo("<div class='alert alert-danger' role='alert'><h3>Wrong password or email!</h3></div>");
            }
...
<body>
<h1 class="jumbotron text-center">ASSISTANT</h1>
